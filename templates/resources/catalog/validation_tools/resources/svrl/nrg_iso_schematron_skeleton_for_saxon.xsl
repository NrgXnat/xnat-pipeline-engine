...
xmlns:validate="http://nrg.wustl.edu/validate" 
...
<!-- ============================================================== -->
	...
	<xsl:template match="iso:assert">
		...
		<axsl:choose>
			<axsl:when test="{@test}">
				<xsl:call-template name="process-passed-assert">
...
<!-- ============================================================== -->
	...
	<xsl:template match="text()" mode="nrg">
		<xsl:value-of select="."/>
	</xsl:template>
	...
	<!-- ISO VALUE-OF -->
	<xsl:template match="iso:value-of" mode="nrg" >
        <xsl:if test="not(@select)">
            <xsl:message><xsl:call-template name="outputLocalizedMessage" ><xsl:with-param name="number">34</xsl:with-param></xsl:call-template></xsl:message>
        </xsl:if>
	    <xsl:call-template name="IamEmpty" />
	...
	<xsl:template match="nrgxsl:acquisition" mode="nrg">
		<nrgxsl:acquisition>		
			<axsl:attribute name="cause-id">
				<xsl:apply-templates select="./nrgxsl:cause-id" mode="nrg" />
 			</axsl:attribute>
			<xsl:apply-templates mode="text" /> 
		</nrgxsl:acquisition>
	</xsl:template>
	
	<xsl:template match="nrgxsl:scans" mode="nrg">
		<nrgxsl:scans>
			<xsl:apply-templates mode="text" /> 
		</nrgxsl:scans>
	</xsl:template>
	...
	<xsl:template match="nrgxsl:scan-id" mode="nrg">
			<xsl:apply-templates mode="text" /> 
	</xsl:template>

	<xsl:template match="nrgxsl:scan" mode="nrg">
		<nrgxsl:scan>
			<axsl:attribute name="id">
				<xsl:apply-templates select="./nrgxsl:scan-id" mode="nrg" />
 			</axsl:attribute>
			<axsl:attribute name="cause-id">
				<xsl:apply-templates select="./nrgxsl:cause-id" mode="nrg" />
 			</axsl:attribute>
			<axsl:attribute name="xmlpath">
				<xsl:apply-templates select="./nrgxsl:xmlpath" mode="nrg" />
 			</axsl:attribute>
		     <xsl:apply-templates mode="text" /> 
